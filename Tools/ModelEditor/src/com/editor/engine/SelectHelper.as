package com.editor.engine
{
	...
	public class SelectHelper
	{
		...
		{
			if(ModelEditorConstants.axis_x == name
				|| ModelEditorConstants.axis_y == name
				|| ModelEditorConstants.axis_z == name
				|| ModelEditorConstants.axis_o == name)
			{
				return true;
			}
			return false;
		...
		public function addHelper( asset:Loader3D ):void 
		{
			...
			for (var i : int = 0; i < asset.numChildren; ++i) {
				...
				if(i == 1){
					...
					mesh.name = ModelEditorConstants.axis_x;
				}				
				if(i == 2){
					mesh.material = new ColorMaterial(0x00aa00);
					mesh.name = ModelEditorConstants.axis_y;
				}
				if(i == 3){
					mesh.material = new ColorMaterial(0x0000aa);
					mesh.name = ModelEditorConstants.axis_z;
				}
				if(i == 0){
					mesh.material = new ColorMaterial(0x999999);
					mesh.name = ModelEditorConstants.axis_o;
				}
		...
		private function onMouseOut(event : MouseEvent3D) : void
		{
			var mesh : Mesh = Mesh(event.target);
			ColorMaterial (mesh.material).color /=2 
			_axis = ModelEditorConstants.axis_n;
		}
