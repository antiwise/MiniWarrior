package com.editor.engine.scene
{
	...
	import com.editor.engine.abstracts.*;
	import com.editor.engine.camera.*;
	import com.editor.engine.events.*;
	import com.editor.engine.geometry.*;
	import com.editor.engine.interfaces.*;
	import com.editor.engine.material.*;
	import com.editor.engine.utils.*;
	import com.editor.engine.vo.*;
	...
	public class SceneBuilder extends RenderSystem implements IBuilder , IAssetContainer , ISceneContainer
	{
		private static var _instance:SceneBuilder;
		...
		protected var object3dAssets : Array = [ ] ;
		...
		public static function get instance():SceneBuilder {
			if( !_instance )
		...
		public function importSceneGeometryVO ( geo : SceneGeometryVO ) : void
		{
			var sec:SceneSectionVO = new SceneSectionVO;
			sec.geometry.push(geo);
			this.addSection(sec);
		}
		
		override public function build (sections : Array ) : void
		{
		...
		override public function addSection ( section : SceneSectionVO ) : void
		{
			if ( section.enabled )
			{
				this.addChild ( section.pivot ) ;
		...
		override public function addObject3DAsset ( id : String ,  data:IAsset) : void
		{
			this.object3dAssets[ id ] = data ;
		}
		...
		{
			for each ( var materialData:MaterialBase in materialLibrary )
			{
				materialData[ MaterialAttributes.SMOOTH ] = true ;
			}
		}
		...
		protected function createGeometry ( section : SceneSectionVO ) : void
		{
			for each ( var geometry : SceneGeometryVO in section.geometry )
			{
				...
				/*if ( useDefaultGeometry ){
					if(!geometryFactory) this.createGeometryFactory();
		...
		protected function createMaterial ( geometry : SceneGeometryVO ) : void
		{
			...
			if ( useDefaultMaterial ) {
				if(!materialFactory)this.createMaterialFactory();
		...
		protected function applyExternalAssets ( section : SceneSectionVO , vo : SceneGeometryVO ) : void
		{
			...
			switch ( vo.materialType )
			{
				...
				//	var bitmapData : BitmapData = this.bitmapDataAssets[ vo.assetClass ] ;
					...
					mat.lights = Get3D.instance.renderSystem.lights;
					mat.gloss = 20;
					mat.specular = 2;
					mat.ambientColor = 0x202030;
					mat.ambient = 1;			
					
					vo.materialData = mat.bitmapData ;
					vo.material = mat ;
					vo.mesh .material = mat;
					
					this.addChildToSection(section.pivot, vo.mesh); // add mouse event
			...
			switch ( vo.geometryType )
			{
				case GeometryType.OBJ :
				{
					if ( vo.mesh != null )
					{
						var _material:BitmapMaterial = new BitmapMaterial;
						...
						vo.mesh.material = _material;
						section.pivot.addChild ( vo.mesh ) ;
						//this.applyColladaValues ( vo.mesh , vo.values , vo ) ;
					}
					else if ( vo.assetClass != null )
					{
						var xml : XML = this.colladaAssets[ vo.assetClass ] ;
						var container : ObjectContainer3D  ;
						
						vo.mesh = container as Mesh;
					...
					else if ( vo.assetFile != null )
					{
						...
						AssetLibrary.addEventListener(AssetEvent.ASSET_COMPLETE, onColladaLoadSuccess);
						AssetLibrary.load(new URLRequest(vo.assetFile));
					}
		...
		protected function onColladaLoadSuccess ( event : AssetEvent ) : void
		{
